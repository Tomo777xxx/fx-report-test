...
def _compose_para2_preview_mix() -> str:
    ...
    # ---------- 5) ブレークポイント短句を1本だけ追加 ----------
    # （最優先で文を作る→句点で一度だけ追記）
    bp_sentence, _bp_dbg = _bp_sentence_mix()

    # 「日足のみ」が選ばれているときは軸をD1に矯正（AUTO/H4を上書き）
    try:
...
# ---- ペア変更フック：下書き/スナップショット/一時成果物を確実に無効化 ----
_cur_pair = str(st.session_state.get("pair", ""))
if st.session_state.get("_pair_prev_for_title") != _cur_pair:
    st.session_state["_pair_prev_for_title"] = _cur_pair
    for k in (
        "ai_title_draft", "ai_recall_draft",
        "ai_title_draft_text", "ai_recall_draft_text",
        "preview_report_base", "preview_report_main",
        "para2_final"
    ):
...
# ①：上流の生成結果を優先。存在しない場合は空文字。
p1 = para1_for_build if "para1_for_build" in globals() else (para1 if "para1" in globals() else "")
# セッション側の唯一真実へ同期（この後のガード処理は session_state を参照するため）
st.session_state["para1_for_build"] = str(p1 or "")

# ②：UIでの下書き > 直近プレビュー > 自動生成 の順で決定
p2_source = (
...
def _strlen_ja(s: str) -> int:
    return len(_nfkc(s or ""))
...
def _window_bounds_jst(today: Optional[datetime] = None) -> Tuple[datetime, datetime]:
    ...
    """
    base = today or _now_jst()
    day0 = base.astimezone(_JST).date()
    start = datetime(day0.year, day0.month, day0.day, 0, 0, tzinfo=_JST)
    end   = start + timedelta(days=1, hours=18)  # 翌日18:00
    return start, end
...
def _get_openai_client():
    ...
    if not key:
        return None
    if not _ensure_pkg("openai"):
        _ai_flags()["last_error"] = "openai パッケージの導入に失敗"
        return None
    # 新SDK優先
    try:
        from openai import OpenAI
        cli = OpenAI(api_key=key)
        st.session_state["__openai_client_cached"] = ("new", cli)
        return ("new", cli)
    except Exception:
        pass
    # 旧SDKも試す
    try:
        ...
        _ai_flags()["last_error"] = f"OpenAI init error: {e}"
        return None
...
def _direct_openai_call(prompt: str, model=None, max_tokens=512, temperature=0.2) -> str:
    ...
    kind, client = cli
    try:
        mdl = model or os.environ.get("OPENAI_MODEL") or st.secrets.get("OPENAI_MODEL", "gpt-4o-mini")
    except Exception:
...
def _llm_call(prompt: str, **kw) -> str:
    """
    1) 既存 llm_complete を優先（互換維持）
    2) 無ければ OpenAI を直接叩く（新/旧SDKどちらでも）
    """
...
def _require_llm(action_label: str = "AI処理") -> bool:
    af = _ai_flags()
    af["ready"] = _llm_ready()
    if not af["ready"]:
        st.error(f"🔴 {action_label} を実行できません：LLM未接続です。`OPENAI_API_KEY` を secrets/環境変数に設定し、再実行してください。")
        return False
    return True
...
MODEL_PREF_ORDER = [m for m in MODEL_PREF_ORDER if m]
    ...
    try:
        ...
        for m in (MODEL_PREF_ORDER or [""]):
            ...
            if tmp:
                ...
                out = tmp; used = True; break
        if not used:
            out = (_llm_call(prompt, **kw) or "").strip()
        if out:
            af["llm_used"] = True
        af["llm_calls"] = int(af.get("llm_calls", 0)) + 1
        af["tokens_est"] = int(af.get("tokens_est", 0)) + _est_tokens(prompt) + _est_tokens(out)
        return out
    except Exception as e:
        af["last_error"] = f"{type(e).__name__}: {e}"
        return ""

def _llm_lamp_inline():
    lamp = "🟢" if _llm_ready() else "🔴"
    st.markdown(f"**LLM接続**：{lamp} {'接続OK' if _llm_ready() else '未接続'}", help="OPENAI_API_KEY（secrets / 環境変数）を確認")

def _ai_usage_lamp_inline():
    af = _ai_flags()
    lamp = "🟢" if af.get("llm_used") else "🔴"
    calls = int(af.get("llm_calls", 0)); toks = int(af.get("tokens_est", 0))
    models = ", ".join(dict.fromkeys(af.get("models_used") or []))
    msg = f"**AI使用状況**：{lamp} 呼び出し {calls} 回 / 概算 {toks} tokens"
    if models: msg += f" / 使用モデル: {models}"
    if af.get("last_error"): msg += f"（最終エラー: {af['last_error']}）"
    st.caption(msg)

with st.expander("LLM診断", expanded=False):
    probe = {}
    try:
        if callable(globals().get("_probe_openai_secrets")):
            probe = globals()["_probe_openai_secrets"]()
    except Exception:
        pass
    if not probe:
        probe = {
            "OPENAI_API_KEY": "set" if bool(_get_api_key()) else "unset",
            "OPENAI_MODEL": os.environ.get("OPENAI_MODEL") or st.secrets.get("OPENAI_MODEL","<unset>")
        }
    st.json(probe)
    st.caption(_ai_flags().get("last_error") or "last_error: <none>")

# -------------------------------------------------
# 2) 表記ゆれ・断定/助言調の中和・辞書ガード
# -------------------------------------------------
NG_WORDS = ["買い","売り","ロング","ショート","損切り","推奨","おすすめ","必勝","間違いない","絶対","必ず","確実","勝てる","儲かる"]
ASSERTIVE_PAT = [
    (r"(上昇|下落)する。", r"\1しやすい。"),
    (r"見込まれる。", "見込む向きもある。"),
    (r"～と断定できる", "～との見方もある"),
...
NORMALIZE_MAP = [
    (r"％", "%"), (r"\s+%", "%"), (r"\s+ドル", "ドル"), (r"\s+円", "円"),
    (r"WTI原油|原油WTI", "原油WTI"),
    (r"ボリンジャーバンド ?[-−]?\s*2σ", "ボリンジャーバンド(20, ±2σ)の下限付近"),
    (r"ボリンジャーバンド ?\+?\s*2σ", "ボリンジャーバンド(20, ±2σ)の上限付近"),
    (r"ボリンジャーバンド ?(中心|ミドル)", "ボリンジャーバンド(20, ±2σ)の中央値（20SMA）付近"),
    (r"２０SMA|20ＳＭＡ|20SＭＡ", "20SMA"),
...
def _neutralize_tone(s: str) -> str:
    t = _clean_text_jp_safe(s)
    for w in NG_WORDS: t = re.sub(w, "（表現を中立化）", t)
    for pat, rep in ASSERTIVE_PAT: t = re.sub(pat, rep, t)
...
def _collect_indicator_terms() -> List[str]:
    ...
    for e in kept:
        ...
        terms.append(raw)
        # 先頭の国・区切り「・」除去バリアント
        terms.append(re.sub(r"^[^・･]+[・･]\s*", "", raw))
        # 括弧内キーワード
        for m in re.findall(r"[（(]([^）)]+)[）)]", raw):
            frag = _nfkc(m).strip()
            if frag:
                terms.append(frag)
                for part in re.split(r"[／/・･\s]", frag):
                    p = _nfkc(part).strip()
                    if p: terms.append(p)
        # 大文字略称
        for m in re.findall(r"\b[A-Z]{2,6}\b", raw):
            terms.append(m)
        # 代表的な和語
        for key in ["消費者物価","雇用統計","小売売上高","国内総生産","政策金利","議事要旨","声明","記者会見","要人発言",
                    "CPI","PCE","コアPCE","ISM","PMI","GDP","NFP","HICP","IFO","ZEW","JOLTS","原油在庫","OPEC","WTI"]:
            if key in raw: terms.append(key)
    # 整理
    out, seen = [], set()
    for t in sorted({ _nfkc(x).strip() for x in terms if _nfkc(x).strip() }, key=lambda z: len(z), reverse=True):
...
def _mask_sensitive(text: str, extra_terms: Optional[List[str]] = None):
    ...
    # 2) DATE / TIME / NUM
    s = re.sub(_TOKEN_PATTERNS["DATE"], _make_repl("DATE"), s)
    s = re.sub(_TOKEN_PATTERNS["TIME"], _make_repl("TIME"), s)
    s = re.sub(_TOKEN_PATTERNS["NUM"],  _make_repl("NUM"),  s)
...
def _unmask_and_verify(llm_out: str, seq, order) -> Tuple[str, bool, str]:
    ...
    if order and got != order:
        return "", False, "placeholder order/count mismatch"
    # 復元：order の順で置換
    for idx, ph in enumerate(order):
        try:
            token = seq[idx][1]
        except Exception:
            return "", False, "placeholder seq mismatch"
        out = out.replace(ph, token, 1)
    return out, True, ""
...
def _fit_title_soft(title: str, target_min=18, target_max=28) -> str:
    ...
    if not t: return t
    if target_min <= len(t) <= target_max: return _dedup_tail(t)
    ...
    if len(t) > target_max and _llm_ready():
        out = _call_llm_with_flags(
            "次のタイトルを、日本語で自然さを保ったまま18〜28字程度に短縮してください。助言・断定は避け、句点は付けない：\n" + t,
            max_tokens=60, temperature=0.2)
        t2 = _clean_text_jp_safe(out).strip("。")
        if 12 <= len(t2) <= 32: t = t2
    if len(t) < target_min: t += "の動向に注視"
    return _dedup_tail(t[:target_max])

# -------------------------------------------------
# 4) 段落③ 一行保証＋回収補強＋長さ予算制御
# -------------------------------------------------
...
def _shrink_text_keep_facts(text: str, min_len: int, max_len: int, extra_terms: Optional[List[str]] = None) -> str:
    ...
    if min_len <= _strlen_ja(base) <= max_len or not _llm_ready():
        return base
    masked, seq, order = _mask_sensitive(base, extra_terms=extra_terms)
    prompt = (
        f"次の日本語文を{min_len}〜{max_len}字程度に自然に要約してください。"
        "数値/日付/時刻/指標名は絶対に変更しないため、同じプレースホルダを保ってください。"
        "助言や断定は禁止。末尾は句点：\n" + masked
    )
    out = _call_llm_with_flags(prompt, max_tokens=480, temperature=0.2)
    unmasked, ok, _ = _unmask_and_verify(out or masked, seq, order)
    return _strict_style_guard(unmasked if ok and unmasked else base)
...
def _rank_news(items: List[dict], max_items=10) -> List[dict]:
    def _norm_title(t: str) -> str:
        ...
        return re.sub(r"[【\[][^】\]]+[】\]]", "", t)
    WEIGHTS = {"FRB":8,"FOMC":8,"パウエル":7,"米雇用統計":9,"NFP":9,"CPI":7,"PCE":7,"ISM":6,"JOLTS":5,"米金利":7,
               "ECB":8,"ラガルド":6,"ユーロ圏":7,"HICP":6,"PMI":5,"ドイツ":5,"IFO":5,"ZEW":5,
               "日銀":9,"植田":6,"YCC":6,"長期金利":5,"消費者物価":5,"為替介入":7,"マイナス金利":6,
               "原油":5,"WTI":5,"OPEC":5,"中東":6,"地政学":6,"停戦":6,"戦闘":6,"停電":5,"地震":5,"為替":3,"外為":3}
    def score(title: str) -> int: return sum(w for k, w in WEIGHTS.items() if k in title)
    seen, ranked = set(), []
    for it in items:
        t = _norm_title(_safe_get(it, "title", "")); 
        if not t or t in seen: continue
        seen.add(t); ranked.append({**it, "title": t, "_score": score(t)})
...
def _fetch_fx_related_news(max_items=10) -> List[dict]:
    ...
    core = ["FRB FOMC 為替","米雇用統計 NFP","米 CPI インフレ","米 PCE 物価","ISM 指数 為替",
...
def _parse_dt_jst(row) -> Optional[datetime]:
    ...
    if not s: return None
    s2 = _nfkc(s)
    m = re.search(r"(\d{4})[-/](\d{1,2})[-/](\d{1,2}).*?([0-2]?\d):([0-5]\d)", s2)
    if m:
        y,mn,d,hh,mm = map(int, m.groups())
        try: return datetime(y,mn,d,hh,mm, tzinfo=_JST)
        except Exception: pass
    m = re.search(r"T([0-2]?\d):([0-5]\d)", s2)
    if m:
        hh,mm = map(int, m.groups())
        base = _now_jst().date()
        return datetime(base.year, base.month, base.day, hh, mm, tzinfo=_JST)
    m = re.search(r"\b([0-2]?\d):([0-5]\d)\b", s2)
    if m:
        hh,mm = map(int, m.groups())
        base = _now_jst().date()
        return datetime(base.year, base.month, base.day, hh, mm, tzinfo=_JST)
...
# 重要度スコア（通貨ペア文脈で加点）
_KEY_SCORES = [
    (r"(FOMC|連邦公開市場委員会|政策金利|利上げ|利下げ|議事要旨|声明|記者会見|要人発言|パウエル|ラガルド|植田|総裁)", 95),
    (r"(CPI|消費者物価|PCE|コアPCE|HICP|インフレ)", 90),
    (r"(雇用統計|NFP|失業率|賃金|JOLTS)", 88),
    (r"(ISM|PMI|製造業|非製造業|サービス業)", 78),
    (r"(小売売上高|小売販売|GDP|国内総生産)", 82),
    (r"(原油在庫|EIA|API|OPEC|WTI)", 76),
    (r"(入札|T[- ]?Note|国債|利回り)", 72),
    (r"(貿易収支|経常収支|財政収支)", 60),
    (r"(NFIB|MBA|レッドブック|卸売在庫|マネーサプライ)", 40),
]

_LOW_IMPORT_PAT = re.compile(r"(NFIB|MBA|レッドブック|卸売在庫|マネーサプライ)", re.I)

def _pair_bias(pair: str, label: str) -> int:
    p = (pair or "").upper()
    lb = _nfkc(label)
    b = 0
    if "USD" in p or "ドル" in p:
        if "米" in lb: b += 8
    if "JPY" in p or "円" in p:
        if "日" in lb: b += 8
    if "EUR" in p or "ユーロ" in p:
        if "欧" in lb or "ユーロ" in lb: b += 6
    if "GBP" in p or "ポンド" in p:
        if "英" in lb: b += 6
    return b

def _importance_score(label: str, pair: str) -> int:
    score = 0
    for pat, w in _KEY_SCORES:
        if re.search(pat, label, flags=re.I):
            score = max(score, w)
    score += _pair_bias(pair, label)
    return score

def _event_label_raw(r: dict) -> str:
    """
    ラベルの“原文”を取得（時刻以外は一切整形しない）
    """
    for k in ["line","display","text","title","指標","name"]:
        if k in r and str(r[k]).strip():
            return str(r[k])
    return ""

def _select_events_for_window(max_items: int = 6):
    """
    本日0:00〜翌日18:00（JST）の範囲で、低重要度を除外しつつ重要順に最大max_items。
    監査用に events_in/kept/dropped を返す。
    """
    src = _events_df_like()
    events_in, kept, dropped = [], [], []
    if src is None:
    ...
    # 収集（同一行の二重連結を防止：H:MM に<原文> の完全一致で一意化）
    seen_lines = set()
    for r in records:
        dt = _parse_dt_jst(r)
        ...
        if not dt or not raw: 
            if not dt:
                dropped.append({"reason":"no-datetime", "row": r})
            elif not raw:
                dropped.append({"reason":"no-label", "row": r})
            continue
        hhmm = _fmt_hhmm(dt, _pick(r, ["time","local_time","datetime","start_at","start"], ""))  # H:MM 固定
        if not hhmm:
            dropped.append({"reason":"no-time", "row": r})
            continue
        line = f"{hhmm} に{raw}"

        # 範囲・重要度の前に、完全同一行の重複をドロップ
        if line in seen_lines:
            dropped.append({"reason":"duplicate-line", "line": line})
            continue
        seen_lines.add(line)

        rec = {"dt": dt, "label_raw": raw, "line": line}
        if not (start <= dt <= end):
            dropped.append({"reason":"out-of-range", "line": line, "dt": dt})
            continue
        if _LOW_IMPORT_PAT.search(raw or ""):
            dropped.append({"reason":"low-importance", "line": line, "dt": dt})
            continue
        sc = _importance_score(raw, pair)
        rec["_score"] = sc
        events_in.append(rec)
...
def _build_calendar_events_and_line(max_items: int = 6):
    ...
    for e in kept:
        ln = e.get("line","")
        if ln in dedup_lines:
            dropped.append({"reason":"duplicate-line-post", "line": ln})
            continue
        dedup_lines.add(ln)
        evs.append(e)

    def _join_lines(evlist):
        return "、".join([e["line"] for e in evlist])

    base_prefix = "本日の指標は、"
    base_suffix = "が発表予定となっている"
    while True:
        cal = _join_lines(evs)
...
# -------------------------------------------------
_cur_pair = str(st.session_state.get("pair", ""))
if st.session_state.get("__pair_prev_ai_title") != _cur_pair:
    st.session_state["__pair_prev_ai_title"] = _cur_pair
    for k in ("p2_ui_preview_text","para2","para2_for_build","para2_final",
              "ai_title_draft","ai_recall_draft","ai_title_draft_text","ai_recall_draft_text",
              "preview_report_base","preview_report_main"):
        st.session_state.pop(k, None)
    if "_para2_preview" in globals():
        try: del globals()["_para2_preview"]
        except Exception: pass
...
def _ai_title_and_recall(preview_text: str, manual_news_list: List[str], picked_news_list: List[dict],
    ...
    def _clean_for_prompt(t: str) -> str:
        try: return _clean_news_title_for_prompt(t)
        except Exception: return str(t)
    for d in (picked_news_list or []):
        title = (d or {}).get("title")
        if title: news_lines.append(_clean_for_prompt(title))
    prompt = (
        "次の素材を踏まえ、FXレポートの『自然で簡潔なタイトル（18〜28字程度）』を1つと、"
        "『タイトル回収の一文（50〜90字程度）』を1つ作成。断定・助言は避け、句読点は和文。媒体名/日付/URLは本文に書かない：\n\n"
        f"【主役ペア】{pair_name}\n"
    ...
    def _parse_title_recall(text: str) -> Tuple[str, str]:
        ...
        s = re.sub(r"\n```$", "", s, flags=re.S)
        title = ""; recall = ""
        for p in [r"(?im)^\s*Title\s*[：:]\s*(.+)$", r"(?im)^\s*タイトル\s*[：:]\s*(.+)$"]:
            m = re.search(p, s); 
            if m: title = m.group(1).strip(); break
        for p in [r"(?im)^\s*Recall\s*[：:]\s*(.+)$", r"(?im)^\s*(?:回収|要旨|要約)\s*[：:]\s*(.+)$"]:
            m = re.search(p, s); 
...
# -------------------------------------------------
# 10) Step6 UI（自動選抜・AI題の強制適用・状態表示）
# -------------------------------------------------
with st.container():
    ...
    st.checkbox("本日の指標（③）を自動選抜（本日0:00〜翌日18:00 JST）", value=st.session_state.get("calendar_autoselect", True), key="calendar_autoselect")
    st.number_input("③の最大件数（重要順）", min_value=3, max_value=10, value=int(st.session_state.get("calendar_max_items", 6) or 6), step=1, key="calendar_max_items")
    ...
    ttl_default = str(st.session_state.get("title_ai", str(globals().get("ttl_display","") or globals().get("title","") or "")))
    st.text_input("AI提案タイトル（編集可・句点なし推奨）", value=ttl_default, key="title_ai")

    def _on_change_recall_input():
        raw = st.session_state.get("ai_title_recall_input", "")
        st.session_state["ai_title_recall_final"] = _strip_media_brackets(_clean_text_jp_safe(raw).rstrip("。"))
    st.text_area("AI提案：タイトル回収（編集可・句点なし推奨）",
                 value=st.session_state.get("ai_title_recall_final",""),
                 key="ai_title_recall_input", height=80, on_change=_on_change_recall_input)

    st.caption("手入力ニュースを優先。AI候補は未入力分を補完します。")
    st.text_area("本日の重要ニュース（手入力・1行1件）",
                 value=st.session_state.get("manual_news_lines",""),
                 placeholder="例）米CPI / FOMC / 介入観測 など",
    ...
    with colA:
        gen_title = st.button("AIでタイトル案を生成 & 適用", key="btn_ai_title")
    with colB:
    ...
    if gen_title:
        if _require_llm("タイトル案のAI生成"):
            ...
            if st.session_state.get("calendar_autoselect", True):
                _, kept_evs, dropped_evs, cal_line = _build_calendar_events_and_line(max_items=int(st.session_state.get("calendar_max_items",6)))
            ...
            if t:
                # 強制適用（B）
                t_fit = _fit_title_soft(t)
                st.session_state["title_ai"] = t_fit
                st.session_state["ai_title_draft_text"] = t_fit
                st.session_state["ai_recall_draft_text"] = r
                st.session_state["__pending_title_input"] = t_fit
                st.session_state["__pending_recall_input"] = _strip_media_brackets(_clean_text_jp_safe(r).rstrip("。"))
                st.session_state["title_source"] = "ai-applied"
                st.success("AIがタイトル案と回収文を生成・適用しました。")
    ...
    with st.expander("③ 重要イベント：候補→採用→プレビュー", expanded=False):
        try:
            evs_in, evs_kept, evs_drop, cal_line = _build_calendar_events_and_line(max_items=int(st.session_state.get("calendar_max_items",6)))
            st.write({"events_in": [e["line"] for e in evs_in],
                      "events_kept": [e["line"] for e in evs_kept],
                      "events_dropped": evs_drop,
                      "preview_line": cal_line})
        except Exception:
...
def _normalize_tf_words(s: str) -> str:
    t = _nfkc(s)
    t = re.sub(r"(しあし|ひあし)", "日足", t)
    t = re.sub(r"４時間足", "4時間足", t)
    return t

def _dedup_d1_h4_phrasing(s: str) -> str:
    t = _normalize_tf_words(s)
    t = re.sub(r"(4時間足[はでは]*)(?=4時間足[はでは]*)", r"\1、", t)
    t = re.sub(r"([。])\1+", r"\1", t)
    return t
...
def _refine_para2_structured(p2_text: str, pair_name: str) -> str:
    ...
    try:
        ...
        prompt = (
            "次の段落②を『日足→4時間足』の順で整理し、反復を避け、自然な日本語に整えてください。"
            "助言はしない・句点で終える・専門語（RSI/ボリンジャーバンド/SMA/EMA）は保持。"
            "数値/日付/時刻/指標名はプレースホルダを維持：\n"
            f"【通貨ペア】{pair_name}\n【段落②】\n{masked}"
        )
        out = _call_llm_with_flags(prompt, max_tokens=520, temperature=0.2)
        if isinstance(out, str) and out.strip():
            unmasked, ok, _ = _unmask_and_verify(out, seq, order)
            base = unmasked if ok and unmasked else base
    except Exception:
        pass
    s = _normalize_tf_words(base)
    sents = [x for x in re.split(r"[。]+", s) if x.strip()]
    d1 = [x for x in sents if "日足" in x]
    h4 = [x for x in sents if "4時間足" in x]
    other = [x for x in sents if x not in d1 + h4]
    def _ensure_h4_beg(x: str) -> str:
        x = x.strip()
        if not x: return x
        if not re.match(r"^4時間足[はでは]", x):
            x = "4時間足では" + ("" if re.match(r"^[はでは]", x) else " ") + x
        return x
    h4 = [_ensure_h4_beg(x) for x in h4]
    out = "。".join(d1 + h4 + other).strip()
    if out:
        out = _dedup_d1_h4_phrasing(out)
        out = out.rstrip("。") + "。"
    else:
        out = p2_text
    return out
...
def _apply_ai_to_p2_with_ai():
    ...
    ind_terms = _collect_indicator_terms()
    masked, seq, order = _mask_sensitive(src, extra_terms=ind_terms)
    prompt = (
        "次の『段落②（為替テクニカル）』を、事実関係を変えずに自然な日本語へ整えてください。"
        "助言・断定は避け、重複表現を削り、句読点・用語を整えます。"
        "数値/日付/時刻/指標名はプレースホルダを維持。末尾は句点：\n---\n" + masked
    )
    tmp = _call_llm_with_flags(prompt, max_tokens=600, temperature=0.2)
    out_raw = tmp or masked
    unmasked, ok, _ = _unmask_and_verify(out_raw, seq, order)
...
def _expand_recall_if_short(recall: str, ttl: str) -> str:
    ...
    if 50 <= _strlen_ja(r) <= 90 or not _llm_ready():
        return r
    prompt = (
        "次の文を、意味を変えずに日本語で50〜90字程度に自然に拡張してください。"
        "断定・助言は禁止。数値・指標名に触れない。末尾は句点：\n"
        f"タイトル: {ttl}\n"
        f"文: {r or ttl}"
    )
    out = _call_llm_with_flags(prompt, max_tokens=120, temperature=0.2)
...
def _apply_ai_to_title_and_p3():
    ...
    def _make_cal_plus_recall(cal_src: str, ttl: str, preview_text: Optional[str] = None) -> str:
        ...
        if _strlen_ja(p3) > P3_MAX_CHARS and _llm_ready():
            ind_terms = _collect_indicator_terms()
            p3 = _fit_para3_oneline(st.session_state.get("calendar_line",""), _shrink_text_keep_facts(recall, 50, 70, extra_terms=ind_terms), period_only)
        return p3
...
for bp in bp_lines:
    if bp and bp not in para2_pre:
        if para2_pre and not para2_pre.endswith("。"): para2_pre += "。"
        para2_pre += bp
...
para2_pre = re.sub(r"(行方を注視したい。|値動きには警戒したい。|当面は静観としたい。|一段の変動に要注意としたい。|方向感を見極めたい。)$", "", para2_pre).rstrip("。") + "。" + desired
    ...
    with c1:
        if st.button("段落①のみ補正（AI）", key="btn_refine_p1"):
            if _require_llm("段落①のAI補正"):
                _apply_ai_to_p1_only(); st.success("段落①をAIで補正しました。"); _ai_usage_lamp_inline()
    with c2:
        if st.button("段落②のみ補正（AI）", key="btn_refine_p2_ai"):
            if _require_llm("段落②のAI補正"):
                _apply_ai_to_p2_with_ai(); st.success("段落②をAIで補正しました。"); _ai_usage_lamp_inline()
    with c3:
        if st.button("段落③＋タイトルを同時補正（AI）", key="btn_refine_p3_title"):
            if _require_llm("タイトル/③のAI補正"):
                _apply_ai_to_title_and_p3(); st.success("段落③とタイトルをAIで補正しました。"); _ai_usage_lamp_inline()
    with c4:
        if st.button("全部まとめて補正（AI）", key="btn_refine_all"):
            if _require_llm("一括AI補正"):
                _apply_ai_to_p1_only(); _apply_ai_to_p2_with_ai(); _apply_ai_to_title_and_p3()
                st.success("全段落＋タイトルをAIで補正しました。"); _ai_usage_lamp_inline()
...
pre_joined = _render_report_safe(ai_title, para1_pre, para2_pre, para3_pre, points)  # タイトルはAI優先
...
with tab2:
    ...
    if st.button("AI後プレビューをAIで補正（手動・一括）", key="btn_refine_preview_after"):
        if _require_llm("一括AI補正"):
            ...
            st.success("AI補正を適用しました。"); _ai_usage_lamp_inline()
...
def _validate_events_raw_match(kept_events) -> bool:
    for e in kept_events or []:
        line = e.get("line","")
        raw = e.get("label_raw","")
        m = re.search(r"^\s*[0-2]?\d:[0-5]\d\s*に(.+)$", line)
...
def _validate_and_fix_all(p1: str, p2: str, p3: str, ttl: str):
    msgs = []
    p1c = _norm_for_check(p1).replace("\n", "")
    p2c = _norm_for_check(p2).replace("\n", "")
    if len(p1c) < p1_min: msgs.append("段落①が規定文字数に未達")
    if len(p2c) < p2_min: msgs.append("段落②が規定文字数に未達")
    ...
    for e in kept:
        if not (start <= e["dt"] <= end):
            msgs.append("③に範囲外イベントが混入"); break
    if not _validate_low_importance_absent(st.session_state.get("calendar_line","")):
    ...
    # 不改変保証（①②の数値・時刻・指標名）
    ind_terms = _collect_indicator_terms()
    if not _validate_tokens_unchanged(para1_pre, p1, extra_terms=ind_terms):
    ...
    return msgs, _final_sanitize(p1), _final_sanitize(p2), _fit_para3_oneline(
        st.session_state.get("calendar_line",""),
...
ai_text_final = (_compact_final_text(_render_report_safe(ai_title_final, ai_p1_final, ai_p2_final, ai_p3_final, points), ai_title_final) + "\n\n" + ai_p3_final).strip()

if can_save:
    try:
        save_path.write_text(ai_text_final, encoding="utf-8")
        st.success(f"保存しました：{fname}")
        st.download_button("このプレビューをダウンロード",
                           data=ai_text_final.encode("utf-8"),
...
# 監査ログ（I）
try:
    ev_in, ev_kept, ev_drop, cal_line_now = _build_calendar_events_and_line(max_items=int(st.session_state.get("calendar_max_items",6)))
    log = {
        ...
        "checks_failed": viol,
        "ai_flags": _ai_flags(),
        "llm_models": list(dict.fromkeys(_ai_flags().get("models_used") or [])),
        "rss_used": bool(_ai_flags().get("rss_used")),
